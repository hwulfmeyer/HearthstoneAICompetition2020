...
namespace SabberStoneBasicAI.AIAgents.Learning
{
	...
	class DynamicLookaheadv1 : AbstractAgent
	{
		...
		public override void FinalizeGame(Game game, Controller myPlayer)
		{
			...
			if (write)
			{
				...
				while (!success)
				{
					try
					{
						using (FileStream fileStream = new FileStream(folder + "DLAv1_" + myPlayer.HeroClass.ToString() + ".csv", FileMode.Append, FileAccess.Write, FileShare.None))
